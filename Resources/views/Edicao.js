function Edicao(data) {
	...
	var a = Ti.UI.createAlertDialog({
		title : "Deseja deletar a edição?"
	});

	deldBt.addEventListener('click', function(e) {
		a.buttonNames = ['Sim', 'Não'];
		a.cancel = 1;
		a.show();
	 ...
	 */
	function getThumb() {
		var local = data.localThumb;
		var remote = data.thumbnail;
		var remoteA = remote.split("/");
		remoteA.reverse();
		var thumbName = remoteA[0];
		...
		if (local != null) {
			...
			if (localA[0] == remoteA[0]) {
				var imageFile = Ti.Filesystem.getFile(local);
				if (imageFile.exists()) {
					thumb.image = local;
				} else {
					loadThumb(thumbName);
	...
	function loadThumb(fileName) {
		...
		xhr.onload = function() {
			...
			var dir = Ti.Filesystem.externalStorageDirectory + data.id + Ti.Filesystem.separator + fileName;
	...
	function loadFile() {
		...
		xhr.onload = function() {
			...
			// var path = Ti.Filesystem.applicationDataDirectory + _data.id + Ti.Filesystem.separator + "files/";
