function ViewPage(data) {
	...
	self.init = function() {
		...
		if (dataLength > 0) {
			for (var i = 0, cont = data.links.length; i < cont; i++) {
		...
		self.addEventListener('singletap', function(e) {
			...
			Ti.App.fireEvent('page:singletap', {
				x : e.x,
				y : e.y
			});
		});
		...
		//                                                                                                                                        //
		//----------------------------------------------------------------------------------------------------------------------------------------//
		var zoomEvent = function(newWidth, newHeight) {
			if (viewAR < pageAR) {
		...
		self.fitPage = function() {
			...
			if (minPageHeight < self.bounds.height) {
